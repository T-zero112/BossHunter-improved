...
class PrefilterHardGateTests(unittest.TestCase):
    ...
    def test_default_rejects_internship_titles(self):
        ...
        self.assertEqual(score, 0)
        self.assertEqual(reason, "实习岗位")

    def test_default_rejects_additional_internship_titles(self):
        from bosshunter.ai.prefilter import quick_score

        config = {"profile": {"deal_breakers": [], "salary_min": 0}}
        job = {"title": "产品实习生", "jd": "", "salary": "8-12K"}
    ...
    def test_daily_salary_filter_uses_internship_daily_range(self):
        from bosshunter.ai.prefilter import quick_score

        config = {"profile": {"recruitment_type": "internship", "internship_daily_salary_min": 180}}

        score, reason = quick_score({"title": "机器人实习生", "jd": "", "salary": "150-170元/天"}, config)

        self.assertEqual(score, 0)
        self.assertIn("日薪低于硬性要求", reason)

        score, reason = quick_score({"title": "机器人实习生", "jd": "", "salary": "180-220元/天"}, config)
        self.assertEqual(score, 100)
        self.assertEqual(reason, "预筛通过")

    def test_recruitment_type_filter_rejects_obvious_mismatch(self):
        from bosshunter.ai.prefilter import quick_score

        config = {"profile": {"recruitment_type": "internship", "internship_daily_salary_min": 0}}
        job = {"title": "机器人社招工程师", "experience": "3-5年", "jd": "", "salary": "20-30K", "recruitment_type": "experienced"}

        score, reason = quick_score(job, config)

        self.assertEqual(score, 0)
        self.assertIn("招聘类型不匹配", reason)
...
class ConfigPageTests(unittest.TestCase):
    ...
    def test_allow_internship_switch_appears_below_deal_breakers(self):
        ...
        allow_internship_index = self.source.index("额外接受实习岗位")
...
class ConfigSchemaTests(unittest.TestCase):
    ...
    def test_schema_adds_allow_internship_after_deal_breakers(self):
        ...
        self.assertEqual(allow_field["label"], "额外接受实习岗位")
    ...
    def test_schema_includes_salary_filter_controls(self):
        ...
        self.assertEqual(fields["internship_daily_salary_min"]["label"], "最低实习日薪（元/天）")
        self.assertEqual(fields["internship_daily_salary_max"]["label"], "最高实习日薪（元/天）")
