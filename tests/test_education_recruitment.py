...
def test_recruitment_type_classification_is_conservative():
    ...
    assert classify_recruitment_type("机器人实习生", "在校生", "") == "internship"
...
def test_database_stores_internship_recruitment_type():
    with tempfile.TemporaryDirectory() as temporary:
        db = get_db(Path(temporary) / "jobs.db")
        try:
            inserted = insert_job_if_new(db, {
    ...
    assert inserted is True
    assert dict(row) == {"education": "本科", "recruitment_type": "internship"}
...
def test_scoring_prompt_includes_candidate_and_job_recruitment_context():
    prompt = _build_scoring_prompt(
        ...
        "候选人简历",
        {"profile": {"education": "硕士", "recruitment_type": "internship"}},
    )

    assert "最高学历：硕士" in prompt
    assert "求职招聘类型：实习" in prompt
