...
def parse_daily_salary_yuan(salary: str) -> tuple[float, float] | None:
    """Parse common internship daily salary labels into a comparable range."""
    normalized = str(salary or "").strip()
    range_match = re.search(
        r"(\d+(?:\.\d+)?)\s*(?:元)?\s*[-–~至]\s*(\d+(?:\.\d+)?)\s*元?\s*/?\s*(?:天|日)",
        normalized,
    )
    if range_match:
        low, high = (float(value) for value in range_match.groups())
    ...
    single_match = re.search(r"(\d+(?:\.\d+)?)\s*元?\s*/?\s*(?:天|日)", normalized)
    if single_match:
        value = float(single_match.group(1))
        return value, value
    return None
