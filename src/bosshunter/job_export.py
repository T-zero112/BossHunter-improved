...
def _filtered_rows(conn: sqlite3.Connection, filters: dict[str, Any] | None = None) -> list[dict[str, Any]]:
	...
	if recruitment_type:
		if recruitment_type not in {"campus", "experienced", "internship", "unknown"}:
	...
	daily_salary_min = filters.get("daily_salary_min")
	daily_salary_max = filters.get("daily_salary_max")
	try:
		...
		minimum_daily_salary = None if daily_salary_min in (None, "") else float(daily_salary_min)
		maximum_daily_salary = None if daily_salary_max in (None, "") else float(daily_salary_max)
	except (TypeError, ValueError) as exc:
	...
	if minimum_daily_salary is not None and maximum_daily_salary is not None and minimum_daily_salary > maximum_daily_salary:
		raise ValueError("最低日薪不能高于最高日薪")
	if minimum_salary is None and maximum_salary is None and minimum_daily_salary is None and maximum_daily_salary is None:
		return rows
	filtered: list[dict[str, Any]] = []
	for row in rows:
		if minimum_salary is not None or maximum_salary is not None:
			...
			if maximum_salary is not None and job_minimum > maximum_salary:
				continue
		if minimum_daily_salary is not None or maximum_daily_salary is not None:
			daily_salary_range = parse_daily_salary_yuan(str(row.get("salary") or ""))
			if daily_salary_range is None:
				continue
			job_daily_minimum, job_daily_maximum = daily_salary_range
			if minimum_daily_salary is not None and job_daily_maximum < minimum_daily_salary:
				continue
			if maximum_daily_salary is not None and job_daily_minimum > maximum_daily_salary:
				continue
		filtered.append(row)
	return filtered
...
def _row_values(job: dict[str, Any]) -> list[Any]:
	...
	job = {
		**job,
		...
		"recruitment_type_label": {"campus": "校招", "experienced": "社招", "internship": "实习"}.get(
