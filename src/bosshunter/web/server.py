...
from bosshunter.web.resume_template_import import (
	build_template_import_preview,
	create_template_from_import_draft,
)
...
from bosshunter.web.resume_photo_extract import ExtractedResumePhoto, extract_resume_photo
...
scheduled_delivery_lock = Lock()
scheduled_delivery_timers: dict[str, Timer] = {}
RESUME_PHOTO_TYPES = {
	"image/jpeg": ".jpg",
	"image/png": ".png",
	"image/webp": ".webp",
}
RESUME_PHOTO_MAX_BYTES = 5 * 1024 * 1024
RESUME_IMPORT_PHOTO_PREFIX = "resume_import_candidate_"
...
def _scheduled_delivery_path() -> Path:
	return DATA_DIR / "scheduled_delivery.json"


def _load_scheduled_deliveries() -> list[dict]:
	path = _scheduled_delivery_path()
	if not path.exists():
		return []
	try:
...
def _save_scheduled_deliveries(schedules: list[dict]) -> None:
	path = _scheduled_delivery_path()
	active_payload = {"schedules": schedules}
	if not schedules:
		try:
			path.unlink()
	...
	path.write_text(json.dumps(active_payload, ensure_ascii=False, indent=2), encoding="utf-8")


def _parse_scheduled_at(value: str) -> datetime:
	raw = str(value or "").strip()
	if not raw:
		raise ValueError("请选择定时投递时间")
	try:
		return datetime.fromisoformat(raw)
	except ValueError as exc:
		raise ValueError("定时投递时间格式无效") from exc


def _time_in_send_windows(target: datetime, windows: list[str]) -> bool:
	if not windows:
		return True
	target_minutes = target.hour * 60 + target.minute
	valid_windows = 0
	for window in windows:
		parts = str(window or "").strip().split("-")
		if len(parts) != 2:
			continue
		start = SendWindowChecker._parse_time(parts[0])
		end = SendWindowChecker._parse_time(parts[1])
...
def _cancel_scheduled_delivery_timer(schedule_id: str | None = None) -> None:
	if schedule_id is None:
		for timer in scheduled_delivery_timers.values():
			timer.cancel()
		scheduled_delivery_timers.clear()
		return
	timer = scheduled_delivery_timers.pop(schedule_id, None)
	if timer is not None:
		timer.cancel()


def _scheduled_delivery_snapshots() -> list[dict]:
	with scheduled_delivery_lock:
		payload = _load_scheduled_deliveries()
	return sorted(payload, key=lambda item: str(item.get("scheduled_at") or ""))


def _scheduled_delivery_snapshot() -> dict | None:
	schedules = _scheduled_delivery_snapshots()
	active = [item for item in schedules if item.get("status") == "active"]
	return active[0] if active else (schedules[0] if schedules else None)


def _mark_scheduled_delivery(payload: dict, **updates) -> dict:
	updated = {
		**payload,
		**updates,
		"updated_at": datetime.now().isoformat(timespec="seconds"),
	}
	schedules = _load_scheduled_deliveries()
	replaced = False
	...
	_save_scheduled_deliveries(schedules)
	return updated


def _run_scheduled_delivery(schedule_id: str) -> None:
	with scheduled_delivery_lock:
		schedules = _load_scheduled_deliveries()
		payload = next((item for item in schedules if item.get("id") == schedule_id), None)
		if not payload or payload.get("status") != "active":
			return
		job_ids = [str(job_id) for job_id in payload.get("job_ids", []) if str(job_id)]
		payload = _mark_scheduled_delivery(payload, status="running", message="定时投递已启动")
		_cancel_scheduled_delivery_timer(schedule_id)
	if not job_ids:
		with scheduled_delivery_lock:
			_mark_scheduled_delivery(payload, status="failed", message="没有可投递岗位")
		return

	def mark_scheduled_started() -> None:
		db = _get_web_db()
		try:
			for job_id in job_ids:
				update_job_status(db, job_id, "approved")
				add_history(db, job_id, "scheduled_delivery_started", "Web Dashboard 定时投递启动")
		finally:
			db.close()
	...
	try:
		task = task_runner.start(
			"deliver",
			_task_config({"_workbench_job_ids": job_ids, "_workbench_skip_greeting": True}),
			before_start=mark_scheduled_started,
		)
	except TaskAlreadyRunningError as exc:
		with scheduled_delivery_lock:
			_mark_scheduled_delivery(payload, status="failed", message=str(exc))
		return
		...
		return
	with scheduled_delivery_lock:
		_mark_scheduled_delivery(payload, status="dispatched", task_id=task.get("id"), message="定时投递任务已提交")


def _arm_scheduled_delivery_timer(payload: dict | None = None) -> None:
	with scheduled_delivery_lock:
		items = [payload] if payload else _load_scheduled_deliveries()
		active_items = [item for item in items if item and item.get("status") == "active"]
		if payload:
			_cancel_scheduled_delivery_timer(str(payload.get("id")))
		elif not active_items:
			_cancel_scheduled_delivery_timer()
			return
		for current in active_items:
			...
			_cancel_scheduled_delivery_timer(schedule_id)
			try:
				scheduled_at = _parse_scheduled_at(str(current.get("scheduled_at") or ""))
			except ValueError:
				_mark_scheduled_delivery(current, status="failed", message="定时投递时间格式无效")
				continue
			delay = max((scheduled_at - datetime.now()).total_seconds(), 0)
			timer = Timer(delay, _run_scheduled_delivery, args=(schedule_id,))
			timer.daemon = True
			scheduled_delivery_timers[schedule_id] = timer
			timer.start()


def _ensure_scheduled_delivery_timer() -> None:
	with scheduled_delivery_lock:
		active_items = [item for item in _load_scheduled_deliveries() if item.get("status") == "active"]
		missing_items = [
			item for item in active_items
			if not (scheduled_delivery_timers.get(str(item.get("id"))) and scheduled_delivery_timers[str(item.get("id"))].is_alive())
		]
	for item in missing_items:
		_arm_scheduled_delivery_timer(item)
...
def _execute_prepare(task: WorkbenchTask, config: dict) -> None:
	"""Confirm selected jobs and generate greetings without sending them."""
	...
	requested_count = int(report.get("requested_count", len(selected_job_ids)) or 0)
	ready_count = generated_count + skipped_existing
	task.metrics.update({
		"prepare_requested": requested_count,
		"greeting_generated": generated_count,
		"greeting_existing": skipped_existing,
		"greeting_failed": failed_count,
	})
	_log(
		task,
		f"预投递准备完成：{ready_count}/{len(selected_job_ids) or requested_count} 个岗位已具备招呼语，"
		f"新生成 {generated_count}，保留已有 {skipped_existing}",
	)
	if failed_count:
		_log(task, f"{failed_count} 个岗位未生成招呼语，仍保留在待确认列表，可稍后重试。")
...
def _execute_deliver_batch(task: WorkbenchTask, config: dict) -> None:
	...
	if quota_deferred_count:
		_log(task, f"{quota_deferred_count} 个岗位因今日发送额度未执行，已保留在“预投递”")
	if paused_count:
		_log(task, f"{paused_count} 个岗位本轮未执行，已保留在“预投递”")
...
def api_job_search():
	try:
		...
		daily_salary_min = _optional_float_param("daily_salary_min")
		daily_salary_max = _optional_float_param("daily_salary_max")
		...
		if daily_salary_min is not None and daily_salary_max is not None and daily_salary_min > daily_salary_max:
			raise ValueError("最低日薪不能高于最高日薪")
		...
		if recruitment_type and recruitment_type not in {"campus", "experienced", "internship", "unknown"}:
	...
	try:
		...
		if salary_min is not None or salary_max is not None or daily_salary_min is not None or daily_salary_max is not None:
			filtered_rows = []
			for row in rows:
				if salary_min is not None or salary_max is not None:
					...
					if salary_max is not None and job_min > salary_max:
						continue
				if daily_salary_min is not None or daily_salary_max is not None:
					daily_salary_range = parse_daily_salary_yuan(row.get("salary", ""))
					if daily_salary_range is None:
						continue
					job_daily_min, job_daily_max = daily_salary_range
					if daily_salary_min is not None and job_daily_max < daily_salary_min:
						continue
					if daily_salary_max is not None and job_daily_min > daily_salary_max:
						continue
...
def api_workbench():
	_ensure_scheduled_delivery_timer()
	db = _get_web_db()
	try:
		config = load_config(CONFIG_PATH)
		...
		return _json_response({
			...
			"scheduled_delivery": _scheduled_delivery_snapshot(),
			"scheduled_deliveries": _scheduled_delivery_snapshots(),
			"task": status["active"],
...
@app.route("/api/workbench/scheduled-delivery", method="POST")
def api_workbench_schedule_delivery():
	try:
		body = request.json or {}
		job_ids = [str(job_id) for job_id in body.get("job_ids", []) if str(job_id)]
		if not job_ids:
			return _json_response({"error": "请选择要定时投递的岗位"}, 400)
		scheduled_at = _parse_scheduled_at(str(body.get("scheduled_at") or ""))
		now = datetime.now()
		if scheduled_at <= now:
			return _json_response({"error": "定时投递时间必须晚于当前时间"}, 400)
		if scheduled_at > now + timedelta(days=14):
			return _json_response({"error": "定时投递最多可设置未来 14 天"}, 400)

		config = load_config(CONFIG_PATH)
		send_windows = config.get("throttle", {}).get("send_windows", [])
		if isinstance(send_windows, list) and not _time_in_send_windows(scheduled_at, send_windows):
			return _json_response({"error": "定时时间不在发送窗口内，请先在配置里修改发送时间"}, 400)
		...
		try:
			...
			invalid_ids = [job_id for job_id in job_ids if job_id not in row_map]
			if invalid_ids:
				return _json_response({"error": "所选岗位不存在或已进入回收站", "invalid_ids": invalid_ids}, 409)
			unsupported = [
				job_id for job_id in job_ids
				if not platform_supports(str(row_map[job_id]["source_platform"] or "boss"), "deliver")
			]
			if unsupported:
				return _json_response({"error": "所选岗位的平台暂不支持定时投递", "invalid_ids": unsupported}, 403)
			completed_statuses = {"sent", "replied", "resume_sent", "needs_resume", "follow_up_sent"}
			not_ready_ids = [
				job_id for job_id in job_ids
				if str(row_map[job_id]["status"] or "") not in {"ready", "approved", "error"}
			]
			already_sent_ids = [
				job_id for job_id in job_ids
				if str(row_map[job_id]["status"] or "") in completed_statuses
			]
			missing_greeting_ids = [
				job_id for job_id in job_ids
				if not str(row_map[job_id]["greeting"] or "").strip()
			]
			if already_sent_ids or missing_greeting_ids or not_ready_ids:
				return _json_response({
					"error": "所选岗位包含尚未准备好、缺少招呼语或已经投递的岗位，暂不能定时投递",
					"already_sent_ids": already_sent_ids,
					"not_ready_ids": not_ready_ids,
					"missing_greeting_ids": missing_greeting_ids,
				}, 409)
			for job_id in job_ids:
				update_job_status(db, job_id, "approved")
				add_history(db, job_id, "scheduled_delivery", f"Web Dashboard 定时投递：{scheduled_at.isoformat(timespec='minutes')}")
		finally:
			db.close()
		...
		with scheduled_delivery_lock:
			existing = _load_scheduled_deliveries()
			incoming_ids = set(job_ids)
			replaced = [
				item for item in existing
				if item.get("status") == "active"
				and not incoming_ids.isdisjoint({str(job_id) for job_id in item.get("job_ids", [])})
			]
			kept = [
				item for item in existing
				if item.get("status") != "active"
				or incoming_ids.isdisjoint({str(job_id) for job_id in item.get("job_ids", [])})
			]
			for item in replaced:
				_cancel_scheduled_delivery_timer(str(item.get("id")))
			_save_scheduled_deliveries([*kept, payload])
		_arm_scheduled_delivery_timer(payload)
...
@app.route("/api/workbench/scheduled-delivery", method="DELETE")
def api_workbench_cancel_scheduled_delivery():
	with scheduled_delivery_lock:
		schedule_id = str(request.params.get("id") or "").strip()
		schedules = _load_scheduled_deliveries()
		if schedule_id:
			canceled = [item for item in schedules if item.get("id") == schedule_id]
			remaining = [item for item in schedules if item.get("id") != schedule_id]
			_cancel_scheduled_delivery_timer(schedule_id)
			_save_scheduled_deliveries(remaining)
			return _json_response({"success": True, "canceled": bool(canceled)})
		_cancel_scheduled_delivery_timer()
		_save_scheduled_deliveries([])
	return _json_response({"success": True, "canceled": bool(schedules)})
...
@app.route("/api/workbench/prepare-greetings", method="POST")
def api_workbench_prepare_greetings():
	try:
		body = request.json or {}
		job_ids = [str(job_id) for job_id in body.get("job_ids", []) if str(job_id)]
		if not job_ids:
			return _json_response({"error": "请选择要加入预投递的岗位"}, 400)
		...
		if unsupported:
			return _json_response({
				"error": "所选岗位的平台暂不支持预投递发送链路",
				"invalid_ids": unsupported,
			}, 403)
		allowed_statuses = {"ready", "approved", "error"}
		completed_statuses = {"sent", "replied", "resume_sent", "needs_resume", "follow_up_sent"}
		not_ready_ids = [
			str(row["id"])
			for row in rows
		...
		if already_sent_ids or not_ready_ids:
			if already_sent_ids and not not_ready_ids:
				error = "所选岗位已经投递，不能重复加入预投递"
			elif not_ready_ids and not already_sent_ids:
				error = "所选岗位尚未完成评分筛选或人工确认，暂不能加入预投递"
			else:
				error = "所选岗位包含尚未准备好或已经投递的岗位，暂不能加入预投递"
			return _json_response({
				"error": error,
				"already_sent_ids": already_sent_ids,
				"not_ready_ids": not_ready_ids,
		...
		def mark_approved() -> None:
			...
			try:
				for job_id in job_ids:
					update_job_status(db, job_id, "approved")
					add_history(db, job_id, "pre_delivery", "Web Dashboard 加入预投递")
			finally:
				db.close()
		...
		with job_mutation_lock:
			task = task_runner.start(
				"prepare",
				_task_config({"_workbench_job_ids": job_ids}),
				before_start=mark_approved,
			)
		return _json_response(task)
	except TaskAlreadyRunningError as e:
		return _json_response({"error": str(e)}, 409)
...
@app.route("/api/jobs/<job_id>/greeting", method="PUT")
def api_job_greeting_save(job_id):
	body = request.json or {}
	greeting = str(body.get("greeting") or "").strip()
	if not greeting:
		return _json_response({"error": "招呼语不能为空"}, 400)
	if len(greeting) > 500:
		return _json_response({"error": "招呼语过长，请控制在 500 字以内"}, 400)

	db = _get_web_db()
	try:
		row = db.execute("SELECT id, status FROM jobs WHERE id = ? AND deleted_at IS NULL", (job_id,)).fetchone()
		if not row:
			return _json_response({"error": "岗位不存在"}, 404)
		if str(row["status"] or "") in {"sent", "replied", "resume_sent", "needs_resume", "follow_up_sent"}:
			return _json_response({"error": "该岗位已进入后续流程，不能再修改招呼语"}, 409)
...
@app.route("/api/jobs/<job_id>/greeting/regenerate", method="POST")
def api_job_greeting_regenerate(job_id):
	try:
		db = _get_web_db()
		try:
			row = db.execute(
				"SELECT id, status, COALESCE(source_platform, 'boss') AS source_platform FROM jobs WHERE id = ? AND deleted_at IS NULL",
				(job_id,),
			).fetchone()
		finally:
			db.close()
		if not row:
			return _json_response({"error": "岗位不存在"}, 404)
		if not platform_supports(str(row["source_platform"] or "boss"), "deliver"):
			return _json_response({"error": "该平台暂不支持生成预投递招呼语"}, 403)
		if str(row["status"] or "") in {"sent", "replied", "resume_sent", "needs_resume", "follow_up_sent"}:
			return _json_response({"error": "该岗位已进入后续流程，不能重新生成招呼语"}, 409)
		if str(row["status"] or "") not in {"ready", "approved", "error"}:
			return _json_response({"error": "该岗位尚未完成评分筛选或人工确认，暂不能重新生成招呼语"}, 409)

		def mark_for_regeneration() -> None:
			mutation_db = _get_web_db()
			try:
				update_job_status(mutation_db, job_id, "approved")
				add_history(mutation_db, job_id, "greeting_regenerate_requested", "Web Dashboard 重新生成招呼语")
			finally:
				mutation_db.close()
		...
		with job_mutation_lock:
			task = task_runner.start(
				"prepare",
				_task_config({
					"_workbench_job_ids": [job_id],
					"_workbench_force_greeting_regenerate": True,
				}),
				before_start=mark_for_regeneration,
			)
		return _json_response(task)
	except TaskAlreadyRunningError as e:
		return _json_response({"error": str(e)}, 409)
...
def api_config_post():
	try:
		...
		if (
			profile.get("internship_daily_salary_min", 0) > profile.get("internship_daily_salary_max", 0)
			and profile.get("internship_daily_salary_max", 0) > 0
		):
			return _json_response({"error": "internship_daily_salary_min must be <= internship_daily_salary_max"}, 400)
...
def api_resume_upload():
	try:
		...
		extracted_photo = extract_resume_photo(raw_name, content, max_bytes=RESUME_PHOTO_MAX_BYTES)
		...
		profile_config["resume_path"] = str(dest.resolve())
		profile_config.pop("resume_generation_source", None)
		profile_config.pop("resume_version_id", None)
		_write_config(config)
		...
		try:
			photo = _save_extracted_profile_photo(extracted_photo)
		except Exception:
			photo = None
		return _json_response({"success": True, **info, "photo": _resume_photo_response(photo), "photo_extracted": bool(photo)})
	except ResumeUploadError as e:
		return _json_response({"error": str(e)}, 400)
	except Exception as e:
		return _json_response({"error": str(e)}, 500)
...
def _resume_center_result(callback, *, status_code: int = 200):
	db = _get_web_db()
	try:
		return _json_response(callback(db), status_code)
	except KeyError as exc:
		return _json_response({"error": str(exc)}, 404)
	except ValueError as exc:
		return _json_response({"error": str(exc)}, 400)
	except sqlite3.IntegrityError as exc:
		return _json_response({"error": f"数据冲突：{exc}"}, 409)
	except Exception as exc:
		return _json_response({"error": str(exc)}, 500)
	finally:
		db.close()
...
def _resume_photo_response(photo: dict | None):
	if not photo:
		return None
	return {
		**photo,
		"image_url": f"/api/resume/photo/file?t={photo.get('created_at', '')}",
	}


def _resume_asset_dir() -> Path:
	asset_dir = DATA_DIR / "resume_assets"
	asset_dir.mkdir(parents=True, exist_ok=True)
	return asset_dir


def _remove_photo_file(photo: dict | None, *, keep_path: Path | None = None) -> None:
...
def _content_matches_photo_type(content: bytes, mime_type: str) -> bool:
...
def _save_profile_photo_content(content: bytes, mime_type: str, suffix: str, db=None) -> dict:
	asset_dir = _resume_asset_dir()
	dest = asset_dir / f"profile_photo_{uuid4().hex}{suffix}"
	...
	if owned_db:
		db = _get_web_db()
	try:
		old_photo = resume_center.get_profile_photo(db)
		photo = resume_center.save_profile_photo(db, str(dest.resolve()), mime_type, len(content))
	finally:
		if owned_db:
			db.close()
	_remove_photo_file(old_photo, keep_path=dest)
	return photo


def _save_extracted_profile_photo(photo: ExtractedResumePhoto | None) -> dict | None:
	if not photo:
		return None
	return _save_profile_photo_content(photo.content, photo.mime_type, photo.suffix)


def _import_photo_candidate_response(filename: str, mime_type: str, file_size: int) -> dict:
	return {
		"filename": filename,
		"mime_type": mime_type,
		"file_size": file_size,
		"image_url": f"/api/resume/import-photo-preview/{filename}?t={int(time.time())}",
	}


def _save_import_photo_candidate(photo: ExtractedResumePhoto | None) -> dict | None:
	if not photo:
		return None
	filename = f"{RESUME_IMPORT_PHOTO_PREFIX}{uuid4().hex}{photo.suffix}"
	dest = _resume_asset_dir() / filename
	dest.write_bytes(photo.content)
	return _import_photo_candidate_response(filename, photo.mime_type, len(photo.content))


def _import_photo_candidate_path(filename: str) -> Path:
	name = Path(str(filename or "")).name
	if not name.startswith(RESUME_IMPORT_PHOTO_PREFIX):
		raise ValueError("照片候选文件无效")
	if Path(name).suffix.lower() not in {".jpg", ".jpeg", ".png", ".webp"}:
		raise ValueError("照片候选文件格式无效")
	path = (_resume_asset_dir() / name).resolve()
	root = _resume_asset_dir().resolve()
	if path.parent != root:
		raise ValueError("照片候选文件路径无效")
	return path


def _promote_import_photo_candidate(candidate: dict | None, db=None) -> dict | None:
	if not isinstance(candidate, dict):
		return None
	filename = str(candidate.get("filename") or "")
	if not filename:
		return None
	path = _import_photo_candidate_path(filename)
	if not path.is_file():
		return None
	content = path.read_bytes()
	if not content or len(content) > RESUME_PHOTO_MAX_BYTES:
		return None
	mime_type = str(candidate.get("mime_type") or "").split(";", 1)[0].strip().lower()
	if mime_type not in RESUME_PHOTO_TYPES:
		return None
	if not _content_matches_photo_type(content, mime_type):
		return None
	saved = _save_profile_photo_content(content, mime_type, RESUME_PHOTO_TYPES[mime_type], db=db)
	try:
		path.unlink()
	except OSError:
		pass
...
@app.route("/api/resume/center")
def api_resume_center_get():
	def get_center(db):
		payload = resume_center.get_resume_center(db)
		payload["photo"] = _resume_photo_response(payload.get("photo"))
		return payload

	return _resume_center_result(get_center)


@app.route("/api/resume/center/reset", method="POST")
def api_resume_center_reset():
	def reset(db):
		result = resume_center.reset_resume_center(db)
		for raw_path in result.pop("asset_paths", []):
		...
		if asset_dir.is_dir():
			for path in asset_dir.iterdir():
				if path.is_file() and (
					path.name.startswith("profile_photo_")
					or path.name.startswith(RESUME_IMPORT_PHOTO_PREFIX)
				):
					try:
						path.unlink()
					except OSError:
						pass
		result["center"]["photo"] = _resume_photo_response(result["center"].get("photo"))
		return result

	return _resume_center_result(reset)


@app.route("/api/resume/import-preview", method="POST")
def api_resume_import_preview():
	try:
		upload = request.files.get("file")
		if not upload:
			return _json_response({"error": "请选择要导入的旧简历"}, 400)
		content = upload.file.read()
		if not content:
			return _json_response({"error": "旧简历文件为空"}, 400)
		if len(content) > 10 * 1024 * 1024:
			return _json_response({"error": "文件大小超过 10MB 限制"}, 400)
		raw_name = upload.raw_filename or upload.filename
		safe_name, stored_content = prepare_resume_content(raw_name, content)
		markdown = stored_content.decode("utf-8")
		draft = build_resume_import_preview(safe_name, markdown, config=load_config(CONFIG_PATH))
		try:
			draft["photo_candidate"] = _save_import_photo_candidate(
				extract_resume_photo(raw_name, content, max_bytes=RESUME_PHOTO_MAX_BYTES)
			)
		except Exception:
			draft["photo_candidate"] = None
...
@app.route("/api/resume/import-confirm", method="POST")
def api_resume_import_confirm():
	body = _resume_center_body()
	draft = body.get("draft")
	if not isinstance(draft, dict):
		return _json_response({"error": "导入草稿不能为空"}, 400)
	photo_candidate = draft.get("photo_candidate")

	def confirm(db):
		result = apply_resume_import_draft(db, draft)
		photo = _promote_import_photo_candidate(photo_candidate, db=db)
		result["photo"] = _resume_photo_response(photo)
		return result

	return _resume_center_result(confirm)


@app.route("/api/resume/template-import-preview", method="POST")
def api_resume_template_import_preview():
	try:
		upload = request.files.get("file")
		if not upload:
			return _json_response({"error": "请选择要导入的模板文件"}, 400)
		content = upload.file.read()
		if not content:
			return _json_response({"error": "模板文件为空"}, 400)
		if len(content) > 10 * 1024 * 1024:
			return _json_response({"error": "文件大小超过 10MB 限制"}, 400)
		raw_name = upload.raw_filename or upload.filename
		safe_name, stored_content = prepare_resume_content(raw_name, content)
		return _json_response(build_template_import_preview(safe_name, stored_content.decode("utf-8")))
	except ResumeUploadError as exc:
		return _json_response({"error": str(exc)}, 400)
	except ValueError as exc:
		return _json_response({"error": str(exc)}, 400)
...
@app.route("/api/resume/template-import-confirm", method="POST")
def api_resume_template_import_confirm():
	body = _resume_center_body()
	draft = body.get("draft")
	if not isinstance(draft, dict):
		return _json_response({"error": "模板草稿不能为空"}, 400)
	return _resume_center_result(lambda db: create_template_from_import_draft(db, draft), status_code=201)


@app.route("/api/resume/profile")
def api_resume_profile_get():
	return _resume_center_result(resume_center.get_profile)


@app.route("/api/resume/profile", method="PUT")
def api_resume_profile_update():
	return _resume_center_result(lambda db: resume_center.update_profile(db, _resume_center_body()))


@app.route("/api/resume/photo")
def api_resume_photo_get():
	return _resume_center_result(lambda db: _resume_photo_response(resume_center.get_profile_photo(db)))


@app.route("/api/resume/import-photo-preview/<filename>")
def api_resume_import_photo_preview(filename):
	try:
		path = _import_photo_candidate_path(filename)
	except ValueError as exc:
		abort(404, str(exc))
	if not path.is_file():
		abort(404, "Resume import photo candidate not found")
	mime_type = mimetypes.guess_type(path.name)[0] or "application/octet-stream"
	return static_file(path.name, root=str(path.parent), mimetype=mime_type)


@app.route("/api/resume/photo", method="POST")
def api_resume_photo_upload():
	try:
		upload = request.files.get("file")
		if not upload:
			return _json_response({"error": "请选择要上传的照片"}, 400)
		content = upload.file.read()
		if not content:
			return _json_response({"error": "照片文件为空"}, 400)
		if len(content) > RESUME_PHOTO_MAX_BYTES:
			return _json_response({"error": "照片大小超过 5MB 限制"}, 400)
		mime_type = str(upload.content_type or "").split(";", 1)[0].strip().lower()
		if mime_type not in RESUME_PHOTO_TYPES:
			return _json_response({"error": "仅支持 JPG、PNG 或 WebP 照片"}, 400)
		if mime_type == "image/jpeg" and not content.startswith(b"\xff\xd8"):
			return _json_response({"error": "照片内容不是有效的 JPG 文件"}, 400)
		if mime_type == "image/png" and not content.startswith(b"\x89PNG\r\n\x1a\n"):
			return _json_response({"error": "照片内容不是有效的 PNG 文件"}, 400)
		if mime_type == "image/webp" and not (content.startswith(b"RIFF") and content[8:12] == b"WEBP"):
			return _json_response({"error": "照片内容不是有效的 WebP 文件"}, 400)

		photo = _save_profile_photo_content(content, mime_type, RESUME_PHOTO_TYPES[mime_type])
		return _json_response({"success": True, "photo": _resume_photo_response(photo)})
	except ValueError as exc:
		return _json_response({"error": str(exc)}, 400)
	except Exception as exc:
		return _json_response({"error": str(exc)}, 500)
...
@app.route("/api/resume/photo", method="DELETE")
def api_resume_photo_delete():
	db = _get_web_db()
	try:
		photo = resume_center.delete_profile_photo(db)
	finally:
		db.close()
...
@app.route("/api/resume/photo/file")
def api_resume_photo_file():
	db = _get_web_db()
	try:
		photo = resume_center.get_profile_photo(db)
	finally:
		db.close()
	if not photo or not photo.get("path"):
		abort(404, "No resume photo configured")
	path = Path(str(photo["path"]))
	if not path.is_file():
		abort(404, "Resume photo not found")
	return static_file(path.name, root=str(path.parent), mimetype=photo.get("mime_type") or None)


@app.route("/api/resume/sections")
def api_resume_sections_get():
	return _resume_center_result(resume_center.list_sections)


@app.route("/api/resume/sections", method="POST")
def api_resume_sections_create():
	return _resume_center_result(lambda db: resume_center.create_section(db, _resume_center_body()), status_code=201)


@app.route("/api/resume/sections/<section_id>", method="PUT")
def api_resume_sections_update(section_id):
	return _resume_center_result(lambda db: resume_center.update_section(db, section_id, _resume_center_body()))


@app.route("/api/resume/sections/<section_id>", method="DELETE")
def api_resume_sections_delete(section_id):
	def delete(db):
		resume_center.delete_section(db, section_id)
		return {"success": True}

	return _resume_center_result(delete)


@app.route("/api/resume/entries", method="POST")
def api_resume_entries_create():
	return _resume_center_result(lambda db: resume_center.create_entry(db, _resume_center_body()), status_code=201)


@app.route("/api/resume/entries/<entry_id>")
def api_resume_entries_get(entry_id):
	return _resume_center_result(lambda db: resume_center.get_entry(db, entry_id))


@app.route("/api/resume/entries/<entry_id>", method="PUT")
def api_resume_entries_update(entry_id):
	return _resume_center_result(lambda db: resume_center.update_entry(db, entry_id, _resume_center_body()))


@app.route("/api/resume/entries/<entry_id>", method="DELETE")
def api_resume_entries_delete(entry_id):
	def delete(db):
		resume_center.delete_entry(db, entry_id)
		return {"success": True}

	return _resume_center_result(delete)


@app.route("/api/resume/materials", method="POST")
def api_resume_materials_create():
	return _resume_center_result(lambda db: resume_center.create_material(db, _resume_center_body()), status_code=201)


@app.route("/api/resume/materials/<material_id>")
def api_resume_materials_get(material_id):
	return _resume_center_result(lambda db: resume_center.get_material(db, material_id))


@app.route("/api/resume/materials/<material_id>", method="PUT")
def api_resume_materials_update(material_id):
	return _resume_center_result(lambda db: resume_center.update_material(db, material_id, _resume_center_body()))


@app.route("/api/resume/materials/<material_id>", method="DELETE")
def api_resume_materials_delete(material_id):
	def delete(db):
		resume_center.delete_material(db, material_id)
		return {"success": True}
...
def api_resume_templates_get():
	return _resume_center_result(resume_center.list_templates)


@app.route("/api/resume/templates", method="POST")
def api_resume_templates_create():
	return _resume_center_result(lambda db: resume_center.create_template(db, _resume_center_body()), status_code=201)


@app.route("/api/resume/templates/<template_id>", method="DELETE")
def api_resume_templates_delete(template_id):
	def delete(db):
		resume_center.delete_template(db, template_id)
		return {"success": True}
...
def api_resume_versions_get():
	return _resume_center_result(resume_center.list_versions)


@app.route("/api/resume/versions", method="POST")
def api_resume_versions_create():
	return _resume_center_result(lambda db: resume_center.create_version(db, _resume_center_body()), status_code=201)


@app.route("/api/resume/versions/duplicate", method="POST")
def api_resume_versions_duplicate_body():
	body = _resume_center_body()
	version_id = str(body.get("version_id") or "").strip()
	if not version_id:
		return _json_response({"error": "version_id 不能为空"}, 400)
	return _resume_center_result(
		lambda db: resume_center.duplicate_version(db, version_id, body),
		status_code=201,
	)


@app.route("/api/resume/export-pdf")
def api_resume_export_pdf():
	version_id = str(request.query.get("version_id") or "").strip()
	if not version_id:
		return _json_response({"error": "version_id 不能为空"}, 400)
	template_id = str(request.query.get("template_id") or "").strip() or None
	show_photo = str(request.query.get("show_photo") or "1").strip().lower() not in {"0", "false", "no"}
	db = _get_web_db()
	try:
		version = resume_center.get_version(db, version_id)
		if template_id:
			snapshot = dict(version.get("profile_snapshot") or {})
			snapshot["template"] = resume_center.get_template(db, template_id)
			version = {**version, "profile_snapshot": snapshot}
	finally:
		db.close()
	try:
		pdf_path = export_resume_version_pdf(
			version,
			DATA_DIR / "resume_exports",
...
@app.route("/api/resume/use-version", method="POST")
def api_resume_use_version():
	try:
		body = _resume_center_body()
		version_id = str(body.get("version_id") or "").strip()
		if not version_id:
		...
		RESUME_DIR.mkdir(parents=True, exist_ok=True)
		filename = resume_version_markdown_filename(version)
		dest = RESUME_DIR / filename
		write_resume_artifacts(dest, markdown_bytes, original_pdf_bytes=None)

		config = load_config(CONFIG_PATH)
		profile_config = config.setdefault("profile", {})
		profile_config["resume_path"] = str(dest.resolve())
		profile_config["resume_generation_source"] = "resume_center_version"
		profile_config["resume_version_id"] = version_id
		_write_config(config)

		db = _get_web_db()
		try:
			resume_center.update_version(
				db,
				version_id,
				{
					**version,
		...
		return _json_response({"success": True, "version_id": version_id, **info})
	except KeyError as exc:
		return _json_response({"error": str(exc)}, 404)
	except ValueError as exc:
		return _json_response({"error": str(exc)}, 400)
...
@app.route("/api/resume/customize-from-job", method="POST")
def api_resume_customize_from_job():
	try:
		body = _resume_center_body()
		job_id = str(body.get("job_id") or "").strip()
		if not job_id:
			return _json_response({"error": "job_id 不能为空"}, 400)
		template_id = str(body.get("template_id") or "").strip() or None
		version_name = str(body.get("version_name") or "").strip() or None
		db = _get_web_db()
		try:
			jobs, _ = query_jobs(db, deleted="all", job_ids=[job_id], limit=1, offset=0)
			if not jobs:
				return _json_response({"error": "岗位不存在"}, 404)
			if template_id:
				resume_center.get_template(db, template_id)
			return _json_response(
				create_resume_version_for_job(
					db,
					jobs[0],
					template_id=template_id,
...
@app.route("/api/resume/versions/<version_id>/duplicate", method="POST")
def api_resume_versions_duplicate(version_id):
	return _resume_center_result(
		lambda db: resume_center.duplicate_version(db, version_id, _resume_center_body()),
		status_code=201,
	)


@app.route("/api/resume/versions/<version_id>")
def api_resume_versions_detail(version_id):
	return _resume_center_result(lambda db: resume_center.get_version(db, version_id))


@app.route("/api/resume/versions/<version_id>", method="PUT")
def api_resume_versions_update(version_id):
	return _resume_center_result(lambda db: resume_center.update_version(db, version_id, _resume_center_body()))


@app.route("/api/resume/versions/<version_id>", method="DELETE")
def api_resume_versions_delete(version_id):
	def delete(db):
		resume_center.delete_version(db, version_id)
		return {"success": True}
