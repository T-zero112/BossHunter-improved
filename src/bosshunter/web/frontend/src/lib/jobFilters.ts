...
export interface JobFilters {
  ...
  salaryMin: string
  salaryMax: string
  dailySalaryMin: string
  dailySalaryMax: string
  status: string
...
export function hasInvalidSalaryRange(filters: JobFilters) {
  const invalidMonthly = filters.salaryMin !== '' && filters.salaryMax !== ''
    && Number(filters.salaryMin) > Number(filters.salaryMax)
  const invalidDaily = filters.dailySalaryMin !== '' && filters.dailySalaryMax !== ''
    && Number(filters.dailySalaryMin) > Number(filters.dailySalaryMax)
  return invalidMonthly || invalidDaily
}
...
function parseDailySalaryYuan(salary: string): [number, number] | null {
  const range = salary.match(/(\d+(?:\.\d+)?)\s*(?:元)?\s*[-–~至]\s*(\d+(?:\.\d+)?)\s*元?\s*\/?\s*(?:天|日)/)
  if (range) {
    const low = Number(range[1])
    const high = Number(range[2])
    return [Math.min(low, high), Math.max(low, high)]
  }
  const single = salary.match(/(\d+(?:\.\d+)?)\s*元?\s*\/?\s*(?:天|日)/)
  if (single) {
    const value = Number(single[1])
    return [value, value]
  }
...
export function filterJobs(jobs: Job[], filters: JobFilters) {
  ...
  const dailySalaryMin = filters.dailySalaryMin === '' ? null : Number(filters.dailySalaryMin)
  const dailySalaryMax = filters.dailySalaryMax === '' ? null : Number(filters.dailySalaryMax)
  const dailySalaryEnabled = dailySalaryMin !== null || dailySalaryMax !== null
  ...
  return jobs.filter(job => {
    ...
    if (dailySalaryEnabled) {
      const dailySalaryRange = parseDailySalaryYuan(job.salary || '')
      if (!dailySalaryRange) return false
      const [jobMin, jobMax] = dailySalaryRange
      if (dailySalaryMin !== null && jobMax < dailySalaryMin) return false
      if (dailySalaryMax !== null && jobMin > dailySalaryMax) return false
    }
    return true
  })
}
