    ...
    <div className="mb-4 rounded-2xl border border-card-border bg-[#FFFCFA] p-3">
      <div className="grid min-w-0 grid-cols-1 gap-2 md:grid-cols-2 2xl:grid-cols-4">
        ...
        <Input
          ...
          min="0"
          step="1"
          value={filters.dailySalaryMin}
          onChange={event => update('dailySalaryMin', event.target.value)}
          placeholder="最低实习日薪"
          className="min-w-0"
          aria-label="最低实习日薪"
        />
        <Input
          ...
          min="0"
          step="1"
          value={filters.dailySalaryMax}
          onChange={event => update('dailySalaryMax', event.target.value)}
          placeholder="最高实习日薪"
          className="min-w-0"
          aria-label="最高实习日薪"
        />
      ...
      {invalidSalary && <p className="mt-2 text-xs font-bold text-danger">最低薪资不能高于最高薪资，请调整月薪或日薪范围。</p>}
    </div>
  )
}
