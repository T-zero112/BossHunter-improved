...
import { BriefcaseBusiness, FileText, Github, LayoutDashboard, Radar, Settings, Terminal } from 'lucide-react'
import { useEffect, useState } from 'react'

const navItems = [
  { to: '/', icon: LayoutDashboard, label: '工作台' },
  { to: '/jobs', icon: BriefcaseBusiness, label: '岗位池' },
  { to: '/resume', icon: FileText, label: '简历中心' },
    ...
    <aside className="w-60 border-r border-card-border bg-white flex flex-col">
      ...
      <div className="px-4 py-4 border-t border-card-border space-y-3">
        <div className="rounded-[8px] border border-card-border bg-[#FFFCFA] p-3">
          ...
          <div className="space-y-1 font-mono text-[10px] leading-4 text-muted">
            <div>cd C:\Users\zx\BossHunter-main</div>
            <div>.\.venv\Scripts\bosshunter.exe web</div>
          </div>
        </div>
        <a
          href={GITHUB_URL}
