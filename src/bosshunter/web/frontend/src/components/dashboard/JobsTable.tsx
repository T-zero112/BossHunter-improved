...
import { CheckCircle2, ChevronDown, ChevronUp, ExternalLink, FileText, Trash2 } from 'lucide-react'
...
interface JobsTableProps {
  ...
  onCreateResumeVersion?: (job: Job) => void
  loading?: boolean
  sortBy: JobSortKey
...
export function JobsTable({ jobs, page, pageSize, total, onPageChange, selectedIds, onToggleSelected, onSoftDelete, onMarkManuallySent, onCreateResumeVersion, loading = false, sortBy, sortOrder, onSortChange }: JobsTableProps) {
  const [expanded, setExpanded] = useState<string | null>(null)
  const [pageInput, setPageInput] = useState(String(page + 1))
  const totalPages = Math.ceil(total / pageSize)
  const hasActions = Boolean(onSoftDelete || onMarkManuallySent || onCreateResumeVersion)
                  ...
                  <Fragment key={job.id}>
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-xs">
                        ...
                        <div className="mt-1 text-muted">{job.recruitment_type === 'campus' ? '校招' : job.recruitment_type === 'experienced' ? '社招' : job.recruitment_type === 'internship' ? '实习' : '类型未识别'}</div>
                      </td>
                      <td className="px-4 py-3">
                        ...
                        <td className="px-3 py-3" onClick={event => event.stopPropagation()}>
                          <div className="flex flex-wrap items-center justify-center gap-1.5">
                            ...
                            {onCreateResumeVersion && (
                              <button
                                type="button"
                                onClick={() => onCreateResumeVersion(job)}
                                className="inline-flex items-center gap-1 rounded-lg border border-card-border px-2 py-1.5 text-[11px] font-bold text-primary hover:bg-[#FFF0E5]"
                              >
                                <FileText className="h-3.5 w-3.5" />生成简历
                              </button>
                            )}
                            {onSoftDelete && (
