...
const initialDrafts: Record<PlatformId, PlatformDraft> = {
  boss: { enabled: true, keywords: '', cities: '', cityCodes: '', maxPages: '3', sort: 'default', recruitmentType: '' },
  zhilian: { enabled: false, keywords: '', cities: '', cityCodes: '', maxPages: '1', sort: 'default', recruitmentType: '' },
  '51job': { enabled: false, keywords: '', cities: '上海', cityCodes: '上海=020000', maxPages: '1', sort: 'default', recruitmentType: '' },
}
...
function recruitmentTypeFromSearch(search: any, config: Record<string, any> | null) {
  const profileType = String(config?.profile?.recruitment_type || '')
  const filters = search.filters && typeof search.filters === 'object' ? search.filters : {}
  const jobTypes = Array.isArray(filters.job_type) ? filters.job_type : []
  if (jobTypes.includes('实习')) return 'internship'
  if (profileType === 'campus' || profileType === 'experienced' || profileType === 'internship') return profileType
  return ''
}

function bossFiltersForRecruitmentType(recruitmentType: string) {
  if (recruitmentType === 'internship') return { job_type: ['实习'] }
  if (recruitmentType === 'campus') return { job_type: ['全职'], experience: ['应届生', '在校生'] }
  if (recruitmentType === 'experienced') return { job_type: ['全职'], experience: ['1年以内', '1-3年', '3-5年', '5-10年', '10年以上'] }
  return {}
}
...
function draftFromConfig(config: Record<string, any> | null, platform: PlatformId): PlatformDraft {
  ...
  return {
    ...
    recruitmentType: platform === 'boss' ? recruitmentTypeFromSearch(configured, config) : '',
  }
}
  ...
  const start = () => {
    ...
    for (const platform of enabledOrder) {
      ...
      platforms[platform] = {
        ...
        sort: draft.sort,
        ...(platform === 'boss' ? { filters: bossFiltersForRecruitmentType(draft.recruitmentType) } : {}),
      }
    }
                ...
                {draft.enabled && <div className="mt-4 space-y-3">
                  ...
                  {platform === 'boss' && (
                    <label className="block text-xs font-bold text-muted">
                      招聘类型
                      <Select value={draft.recruitmentType} onChange={event => updateDraft(platform, 'recruitmentType', event.target.value)}>
