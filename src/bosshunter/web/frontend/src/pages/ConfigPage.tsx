    ...
    <div className="h-full overflow-y-auto space-y-4 pr-4">
        ...
        <SectionCard title="个人信息" sectionKey="profile" expanded={expandedSections} toggle={toggleSection}>
          <div className="space-y-4">
            ...
            <NumberRangeField
              label="实习日薪范围（元/天）"
              minValue={config.profile?.internship_daily_salary_min ?? 0}
              maxValue={config.profile?.internship_daily_salary_max ?? 0}
              onMinChange={value => updateConfig('profile.internship_daily_salary_min', value)}
              onMaxChange={value => updateConfig('profile.internship_daily_salary_max', value)}
              min={0}
              max={2000}
            />
            ...
            <div className="flex items-center justify-between">
              <label className="text-xs text-foreground">额外接受实习岗位</label>
