...
interface Job {
  ...
  experience: string
  education?: string
  recruitment_type?: 'campus' | 'experienced' | 'internship' | 'unknown' | string
  jd: string
  score: number
  score_reason: string
  greeting: string
...
export interface WorkbenchTask {
  id: string
  mode: 'full' | 'collect' | 'rescore' | 'monitor' | 'prepare' | 'deliver'
  label: string
  status: string
  logs: string[]
  error?: string
...
export interface ScheduledDelivery {
  id: string
  status: 'active' | 'running' | 'dispatched' | 'failed' | 'canceled' | string
  scheduled_at: string
  job_ids?: string[]
  job_count?: number
  message?: string
...
export function useDashboard(scope: DashboardDataScope = 'all') {
  ...
  const startTask = async (mode: 'full' | 'collect' | 'rescore' | 'monitor' | 'prepare' | 'deliver', options?: Record<string, unknown>) => {
