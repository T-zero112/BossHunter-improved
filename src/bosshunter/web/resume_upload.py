...
def _paragraph_to_markdown(paragraph: ElementTree.Element) -> str:
	...
	if _paragraph_is_bold_heading(paragraph, text):
		return f"### {text}"
...
def _paragraph_is_bold_heading(paragraph: ElementTree.Element, text: str) -> bool:
	...
	if re.search(r"完成|负责|参与|实现|搭建|优化|分析|记录|使用|基于", text):
		return False
	text_nodes = [node for node in paragraph.findall(f".//{_W}r") if _element_text(node)]
	if not text_nodes:
		return False
	bold_nodes = [
		node for node in text_nodes
		if node.find(f"./{_W}rPr/{_W}b") is not None
	]
	return len(bold_nodes) >= max(1, len(text_nodes) // 2)
