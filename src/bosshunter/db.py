...
def insert_job_if_new(conn: sqlite3.Connection, job: dict[str, Any]) -> bool:
    ...
    values = {
        ...
        "recruitment_type": (
            ...
            if job.get("recruitment_type") in {"campus", "experienced", "internship"}
            else "unknown"
        ),
        "jd": job.get("jd", ""),
...
def _migrate_v1_4(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(f"""
        UPDATE jobs SET recruitment_type = CASE
            WHEN {searchable} LIKE '%实习%' OR LOWER({searchable}) LIKE '%intern%' THEN 'internship'
            WHEN {searchable} LIKE '%校招%' OR {searchable} LIKE '%校园招聘%'
              OR {searchable} LIKE '%应届%' OR {searchable} LIKE '%毕业生%'
              OR {searchable} LIKE '%管培生%' THEN 'campus'
        ...
        WHERE recruitment_type IS NULL OR TRIM(recruitment_type) = '' OR recruitment_type = 'unknown'
           OR (recruitment_type = 'campus' AND ({searchable} LIKE '%实习%' OR LOWER({searchable}) LIKE '%intern%'))
    """)
...
def _init_resume_center(conn: sqlite3.Connection) -> None:
    """Create the structured resume-center store without touching legacy resume_path."""
    conn.executescript(
        """
        ...
        CREATE TABLE IF NOT EXISTS resume_materials (
            ...
            material_type TEXT NOT NULL DEFAULT 'bullet',
            content TEXT NOT NULL,
            evidence TEXT NOT NULL DEFAULT '',
            tags_json TEXT NOT NULL DEFAULT '[]',
            jd_keywords_json TEXT NOT NULL DEFAULT '[]',
        ...
        CREATE TABLE IF NOT EXISTS resume_templates (
            id TEXT PRIMARY KEY,
            name TEXT NOT NULL,
            template_type TEXT NOT NULL DEFAULT 'classic_single',
        ...
        CREATE INDEX IF NOT EXISTS idx_resume_sections_profile_order
            ON resume_sections(profile_id, sort_order, created_at);
        CREATE INDEX IF NOT EXISTS idx_resume_entries_section_order
            ON resume_entries(section_id, sort_order, created_at);
        CREATE INDEX IF NOT EXISTS idx_resume_materials_entry_order
            ON resume_materials(entry_id, sort_order, created_at);
        CREATE INDEX IF NOT EXISTS idx_resume_versions_target_job
            ON resume_versions(target_job_id, updated_at);
        """
    )
    version_cols = {row[1] for row in conn.execute("PRAGMA table_info(resume_versions)").fetchall()}
    version_additions = {
        "selected_section_ids_json": "TEXT NOT NULL DEFAULT '[]'",
        "selected_entry_ids_json": "TEXT NOT NULL DEFAULT '[]'",
    }
    ...
    asset_cols = {row[1] for row in conn.execute("PRAGMA table_info(resume_assets)").fetchall()}
    asset_additions = {
    ...
    for name, definition in asset_additions.items():
        if name not in asset_cols:
            conn.execute(f"ALTER TABLE resume_assets ADD COLUMN {name} {definition}")
    builtin_resume_templates = [
        (
            "classic-single",
            "经典单栏",
            "classic_single",
            {"columns": 1, "density": "standard", "base_template": "classic-single"},
            {"font": "system", "accent": "#FF5A1F"},
            "",
        ),
        (
            "compact-two-column",
            "简洁双栏",
            "compact_two_column",
            {"columns": 2, "density": "compact", "base_template": "compact-two-column"},
            {"font": "system", "accent": "#2563EB"},
            "",
        ),
        (
            "reactive-azurill",
            "Azurill",
            "reactive_resume",
            {"columns": 2, "density": "standard", "base_template": "compact-two-column", "source": "Reactive Resume", "license": "MIT"},
            {"font": "system", "accent": "#3B82F6"},
            "/external-templates/reactive-resume/azurill.jpg",
        ),
        (
            "reactive-bronzor",
            "Bronzor",
            "reactive_resume",
            {"columns": 2, "density": "standard", "base_template": "compact-two-column", "source": "Reactive Resume", "license": "MIT"},
            {"font": "system", "accent": "#4B5563"},
            "/external-templates/reactive-resume/bronzor.jpg",
        ),
        (
            "reactive-chikorita",
            "Chikorita",
            "reactive_resume",
            {"columns": 2, "density": "standard", "base_template": "compact-two-column", "source": "Reactive Resume", "license": "MIT"},
            {"font": "system", "accent": "#7C93B5"},
            "/external-templates/reactive-resume/chikorita.jpg",
        ),
        (
            "reactive-ditgar",
            "Ditgar",
            "reactive_resume",
            {"columns": 2, "density": "compact", "base_template": "compact-two-column", "source": "Reactive Resume", "license": "MIT"},
            {"font": "system", "accent": "#1F4E5F"},
            "/external-templates/reactive-resume/ditgar.jpg",
        ),
        (
            "reactive-ditto",
            "Ditto",
            "reactive_resume",
            {"columns": 2, "density": "compact", "base_template": "compact-two-column", "source": "Reactive Resume", "license": "MIT"},
            {"font": "system", "accent": "#333333"},
            "/external-templates/reactive-resume/ditto.jpg",
        ),
        (
            "reactive-gengar",
            "Gengar",
            "reactive_resume",
            {"columns": 2, "density": "standard", "base_template": "compact-two-column", "source": "Reactive Resume", "license": "MIT"},
            {"font": "system", "accent": "#6D5BD0"},
            "/external-templates/reactive-resume/gengar.jpg",
        ),
        (
            "reactive-glalie",
            "Glalie",
            "reactive_resume",
            {"columns": 2, "density": "standard", "base_template": "compact-two-column", "source": "Reactive Resume", "license": "MIT"},
            {"font": "system", "accent": "#9CA3AF"},
            "/external-templates/reactive-resume/glalie.jpg",
        ),
        (
            "reactive-kakuna",
            "Kakuna",
            "reactive_resume",
            {"columns": 1, "density": "compact", "base_template": "classic-single", "source": "Reactive Resume", "license": "MIT"},
            {"font": "system", "accent": "#C026D3"},
            "/external-templates/reactive-resume/kakuna.jpg",
        ),
        (
            "reactive-lapras",
            "Lapras",
            "reactive_resume",
            {"columns": 1, "density": "standard", "base_template": "classic-single", "source": "Reactive Resume", "license": "MIT"},
            {"font": "system", "accent": "#2563EB"},
            "/external-templates/reactive-resume/lapras.jpg",
        ),
        (
            "reactive-leafish",
            "Leafish",
            "reactive_resume",
            {"columns": 2, "density": "standard", "base_template": "compact-two-column", "source": "Reactive Resume", "license": "MIT"},
            {"font": "system", "accent": "#6B8E6E"},
            "/external-templates/reactive-resume/leafish.jpg",
        ),
        (
            "reactive-meowth",
            "Meowth",
            ...
            {"columns": 1, "density": "compact", "base_template": "classic-single", "source": "Reactive Resume", "license": "MIT"},
            {"font": "system", "accent": "#2563EB"},
            "/external-templates/reactive-resume/meowth.jpg",
        ),
        (
            "reactive-onyx",
            "Onyx",
            "reactive_resume",
            {"columns": 1, "density": "standard", "base_template": "classic-single", "source": "Reactive Resume", "license": "MIT"},
            {"font": "system", "accent": "#111827"},
            "/external-templates/reactive-resume/onyx.jpg",
        ),
        (
            "reactive-pikachu",
            "Pikachu",
            "reactive_resume",
            {"columns": 2, "density": "standard", "base_template": "compact-two-column", "source": "Reactive Resume", "license": "MIT"},
            {"font": "system", "accent": "#F59E0B"},
            "/external-templates/reactive-resume/pikachu.jpg",
        ),
        (
            "reactive-rhyhorn",
            "Rhyhorn",
            "reactive_resume",
            {"columns": 1, "density": "standard", "base_template": "classic-single", "source": "Reactive Resume", "license": "MIT"},
            {"font": "system", "accent": "#334155"},
            "/external-templates/reactive-resume/rhyhorn.jpg",
        ),
        (
            "reactive-scizor",
            "Scizor",
            "reactive_resume",
            {"columns": 1, "density": "standard", "base_template": "classic-single", "source": "Reactive Resume", "license": "MIT"},
            {"font": "system", "accent": "#DC2626"},
            "/external-templates/reactive-resume/scizor.jpg",
        ),
        (
            "rendercv-classic",
            "Classic",
            "rendercv",
            {"columns": 1, "density": "standard", "base_template": "classic-single", "source": "RenderCV", "license": "MIT"},
            {"font": "system", "accent": "#1F2937"},
            "/external-templates/rendercv/classic.png",
        ),
        (
            "rendercv-ember",
            "Ember",
            "rendercv",
            {"columns": 1, "density": "standard", "base_template": "classic-single", "source": "RenderCV", "license": "MIT"},
            {"font": "system", "accent": "#B45309"},
            "/external-templates/rendercv/ember.png",
        ),
        (
            "rendercv-engineeringclassic",
            "Engineering Classic",
            "rendercv",
            {"columns": 1, "density": "compact", "base_template": "classic-single", "source": "RenderCV", "license": "MIT"},
            {"font": "system", "accent": "#2563EB"},
            "/external-templates/rendercv/engineeringclassic.png",
        ),
        (
            "rendercv-engineeringresumes",
            "Engineering Resumes",
            "rendercv",
            {"columns": 1, "density": "compact", "base_template": "classic-single", "source": "RenderCV", "license": "MIT"},
            {"font": "system", "accent": "#111827"},
            "/external-templates/rendercv/engineeringresumes.png",
        ),
        (
            "rendercv-harvard",
            "Harvard",
            "rendercv",
            {"columns": 1, "density": "standard", "base_template": "classic-single", "source": "RenderCV", "license": "MIT"},
            {"font": "system", "accent": "#7F1D1D"},
            "/external-templates/rendercv/harvard.png",
        ),
        (
            "rendercv-ink",
            "Ink",
            "rendercv",
            {"columns": 1, "density": "compact", "base_template": "classic-single", "source": "RenderCV", "license": "MIT"},
            {"font": "system", "accent": "#000000"},
            "/external-templates/rendercv/ink.png",
        ),
        (
            "rendercv-moderncv",
            "ModernCV",
            ...
            {"columns": 1, "density": "standard", "base_template": "classic-single", "source": "RenderCV", "license": "MIT"},
            {"font": "system", "accent": "#2563EB"},
            "/external-templates/rendercv/moderncv.png",
        ),
        (
            "rendercv-opal",
            "Opal",
            "rendercv",
            {"columns": 1, "density": "standard", "base_template": "classic-single", "source": "RenderCV", "license": "MIT"},
            {"font": "system", "accent": "#0F766E"},
            "/external-templates/rendercv/opal.png",
        ),
        (
            "rendercv-sb2nov",
            "Sb2nov",
            ...
            {"columns": 1, "density": "compact", "base_template": "classic-single", "source": "RenderCV", "license": "MIT"},
            {"font": "system", "accent": "#111827"},
            "/external-templates/rendercv/sb2nov.png",
        ),
    ]
    conn.executemany(
        """
        ...
        """,
        [
            (template_id, name, template_type, json.dumps(layout, ensure_ascii=False), json.dumps(style, ensure_ascii=False), preview_path)
            for template_id, name, template_type, layout, style, preview_path in builtin_resume_templates
        ],
    )
    conn.commit()
